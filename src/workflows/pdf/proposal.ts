...
class ProposalPdfFactoryPicker extends PdfFactoryPicker<
  ProposalPDFData,
  ProposalPDFMeta,
  PdfFactory<ProposalPDFData, ProposalPDFMeta>
> {
  public getFactory(data: ProposalPDFData, entityId: number) {
    if (data.pdfTemplateId === undefined) {
      return new ProposalPdfFactory(entityId);
    } else {
      return new CustomProposalPdfFactory(entityId);
    }
  }
}
...
export default function newProposalPdfWorkflowManager(data: ProposalPDFData[]) {
  const manager = new PdfWorkflowManager<
    ProposalPDFData,
    ProposalPDFMeta,
    CustomProposalPdfFactory
  >(new ProposalPdfFactoryPicker(), data, (data) => data.proposal.primaryKey);
